...
if (!dbFile.exists()) {
    console.log(`Creating new embeddings database at ${env.EMBEDDINGS_SQLITE_FILE_PATH}`)
...
export function createVectorDbClient(key: VectorDbClientKey) {
    function readEntry(id: string) {
        const [entry] = db.query("SELECT vector FROM vec_entries WHERE key = ? AND id = ?").all(key, id)
    ...
    function writeEntry(id: string, vector: number[]) {
        ...
        if (exists) {
            db.run("UPDATE vec_entries SET vector = ? WHERE key = ? AND id = ?", [Buffer.from(new Float32Array(vector).buffer), key, id])
        } else {
            db.run("INSERT INTO vec_entries (key, id, vector) VALUES (?, ?, ?)", [key, id, Buffer.from(new Float32Array(vector).buffer)])
        }
    }
    ...
    function all() {
        const rows = db.query("SELECT id, vector FROM vec_entries WHERE key = ?").all(key)

        return z.array(z.object({ id: z.string(), vector: z.instanceof(Uint8Array) })).parse(rows).map(({ id, vector }) => {
            const floatArray = new Float32Array(vector.buffer, vector.byteOffset, vector.byteLength / Float32Array.BYTES_PER_ELEMENT)
            return {
                id,
